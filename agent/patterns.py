...
def _double_candle_patterns(df):
    ...
    # 乌云盖顶
    if _is_bullish(prev) and _is_bearish(last):
        if float(last["open"]) > float(prev["high"]) and float(last["close"]) < (float(prev["open"]) + float(prev["close"])) / 2:
            patterns.append({
                "name": "乌云盖顶",
                "type": "bearish",
                "confidence": 0.65,
    ...
    # 曙光初现
    if _is_bearish(prev) and _is_bullish(last):
        if float(last["open"]) < float(prev["low"]) and float(last["close"]) > (float(prev["open"]) + float(prev["close"])) / 2:
            patterns.append({
                "name": "曙光初现",
                "type": "bullish",
                "confidence": 0.65,
...
def _platform_patterns(df):
    ...
    highs = [float(r["high"]) for _, r in recent.iterrows()]
    ...
    if max_dev < 0.05 and len(closes) >= 10:
        # 检查是否在末端
        last_close = closes[-1]
        last_3_change = (closes[-1] - closes[-3]) / closes[-3] * 100 if closes[-3] > 0 else 0
        patterns.append({
            "name": "横盘整理",
            "type": "neutral",
            "confidence": 0.6,

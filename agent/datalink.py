    ...
    try:
        ...
        for _, row in df.head(30).iterrows():
            stocks.append({
                ...
                "sell_total": round(float(row.get("龙虎榜卖出额", 0)) / 1e8, 2) if row.get("龙虎榜卖出额") else None,
...
def get_northbound_flow(trade_date: str = None) -> dict:
    ...
    try:
        ...
        return {
            ...
            "daily_flows": flows[-5:],
            "trend": "持续流入" if total_recent > 0
                     else "持续流出" if total_recent < 0
                     else "平衡",
        }
    except Exception as e:
...
def get_margin_trading(trade_date: str = None) -> dict:
    ...
    import akshare as ak
    try:
        df = ak.stock_margin_sse(start_date=(date.today() - timedelta(days=10)).strftime("%Y%m%d"),

...
<img width="1730" height="924" alt="image" src="https://github.com/user-attachments/assets/86159939-d360-41da-b171-7c23d5802715" />
...
| **🎨 TUI 全屏界面** | **现代化全屏终端界面，实时流式输出，三栏布局（自选股/对话流/工具面板）** |
| **⚖️ 评判Agent** | **分析完成后用户可手动引入质量检查，发现逻辑漏洞、数据问题、风险遗漏，引导用户纠错** |
...
DeepPulse v0.2 引入**评判Agent**（Judge Agent），在主分析完成后用户可手动选择是否介入，进行质量检查和纠错引导：

**工作原理**：
1. **逻辑检查** — 分析推理是否自洽？结论是否与数据匹配？是否存在逻辑跳跃？
2. **数据验证** — 技术指标计算是否合理？数据时效性是否明确？是否误用历史数据？
3. **风险补充** — 主Agent遗漏的风险点、过度乐观/悲观的倾向、缺失的上下文信息
4. **提问引导** — 建议补充哪些信息？推荐进一步分析的角度？
...
**纠错学习闭环**：
- 用户选择"有问题需要纠正" → 描述具体问题 → 自动记录到`learning`记忆（半衰期180天）
...
**TUI 模式特点**：
- ✨ 三栏布局：左侧自选股/持仓，中间对话流，右侧工具执行面板
- 🔄 实时流式输出：主分析Agent + 评判Agent双流式显示
- 🎯 快捷键操作：Ctrl+Q退出，Ctrl+R重置，Ctrl+L清屏
- 📊 实时数据面板：技术指标、市场概况动态刷新
- ⚖️ 自动质量检查：每次分析后评判Agent自动介入，发现潜在问题
- 📋 文本复制：按住 **Shift 键** + 鼠标选择文本，然后 `Ctrl+Shift+C` 复制（推荐使用 Windows Terminal）

**评判Agent工作流程**：
1. 主分析Agent完成分析后，评判Agent手动选择介入
2. 检查逻辑自洽性、数据准确性、风险遗漏
3. 流式输出发现的问题和改进建议
4. 用户选择：✅ 分析合理 / ❌ 有问题需要纠正 / 💬 继续提问
5. 如果用户纠正，系统自动记录到学习记忆，下次分析时应用
